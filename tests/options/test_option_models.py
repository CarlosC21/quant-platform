...
def test_delta_hedge_pnl_call():
    S0, K, r, sigma = 100, 100, 1.0, 0.05, 0.2
    np.random.seed(42)
    S_path = S0 * np.exp(
        np.cumsum(
    ...
    def option_factory(S, T):
        return BlackScholesOption(S, K, T, r, sigma, option_type="call")
    ...
    def option_factory_amer(S, T):
        return AmericanOption(S, K, T, r, sigma, steps=100, option_type="call")
...
def test_delta_hedge_pnl_put():
    ...
    # Black-Scholes put
    def option_factory(S, T):
        return BlackScholesOption(S, K, T, r, sigma, option_type="put")
    ...
    # American put
    def option_factory_amer(S, T):
        return AmericanOption(S, K, T, r, sigma, steps=100, option_type="put")
